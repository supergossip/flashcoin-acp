...
*Setup instructions for a gitian build of flashcoin using a Debian VM or physical system.*
...
- [Building flashcoin](#building-flashcoin)
...
Clone the git repositories for flashcoin and gitian and then checkout the flashcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the flashcoin repository
...
To build flashcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the flashcoin repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/flashcoin-project/flashcoin
...
For example:
```bash
URL=https://github.com/user/flashcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/flashcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/flashcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/flashcoin-build.assert
```
...
[flashcoin/gitian.sigs](https://github.com/flashcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
